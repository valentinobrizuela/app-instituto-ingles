...
Views.Quizzes = {
    render() {
        ...
        html += courses.map(course => {
            const courseQuizzes = quizzes.filter(q => String() === String());
                    ...
                    <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap:1.5rem;">
                        ...
                        ${courseQuizzes.map(q => {
                            ...
                            if (user.role === 'student') {
                                const result = results.find(r => String() === String() && String(r.student_id) === String(user.id));
                                if (result) {
                                    actionHtml = `
                                ...
                                const totalTaken = results.filter(r => String() === String()).length;
                                actionHtml = `
