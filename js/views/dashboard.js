...
Views.Dashboard = {
    ...
    renderTeacher(user, container) {
        const courses = DB.getTable('courses').filter(c => String(c.teacher_id) === String(user.id));
        const students = DB.getTable('users').filter(u => String(u.teacher_id) === String(user.id));
